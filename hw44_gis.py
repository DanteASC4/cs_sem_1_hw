...
def getData():
     ...
     c_f = input('Enter the name of a csv file:   ')
     df = pd.read_csv(c_f,encoding="utf-8-sig") #<-- placeholder, can remove once defined.
...
def getColumnNames():
     ...
     latName = input('Enter the exact name of the column that contains the latitude coordinates:  ')
     lonName = input('Enter the exact name of the column that contains the longitude coordinates:  ')
